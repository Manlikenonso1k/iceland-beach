<?php 
    ...
    if(isset($_POST['pay'])){
        ...
        $select_pass = $query->select("membership", "password", "email = ?", [$email], "s");

        if($select_pass->num_rows > 0){
            $row = $select_pass->fetch_assoc();
            if(password_verify($_POST['password'], $row['password'])){
                ...
                $request = [
                    ...
                    'redirect_url' => 'https://icelandbeach.com/core/processor/processmember2.php',
                    'customer' => [
                        'email' => $email,
                ...
                echo "<div style='text-align:center;'>Password is incorrect</div>";
...
<div class='content'>
    <form method="post">
        ...
        <small>Renewing Your membership</small>
        <h3>FULLNAME : <?=$_SESSION['fullname']?></h3>
        <h3>MEMBERSHIP TYPE : <?=$_SESSION['type']?></h3>
        <h3>MEMBERSHIP TYPE : <?=$_SESSION['mplan']?></h3>
        <h3>TOTAL BILL : <?=$_SESSION['price']?></h3>
        <input class="forminput" type="password" name="password" placeholder="Input your password" required>
        <button type="submit" name="pay" class="cta-button">Proceed to Payment</button>
        <a href="../../renew.php" class="cta">Back Home</a>
    </form>
</div>
</div>
</div>
